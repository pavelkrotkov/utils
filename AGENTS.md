...
- `uv run ./skills/simplifi-transaction-reconciler/scripts/simplifi_transaction_reconciler.py mutate --db /path/to/review.sqlite` (dry run; `--apply` additionally needs `--authorized-by` and `--authorization-note`)
